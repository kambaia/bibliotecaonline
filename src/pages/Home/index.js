             ...
             <ContentSession style={{
                  backgroundSize: 'cover',
                    ...
                    backgroundImage: ` linear-gradient(to right, rgba(0,0,0,1) 30%, transparent 90%), url(${bainner})`}}>
                     <Categoria titulo="Os mais lidos por todos os estudantes" style={{color:"#fff"}}/>
        ...
        <>
           <ContentCategoria>
            <div className="content">
                <div className="text-categoria"><h2 style={{color: style? '#fff': '#000'}}>{titulo}</h2></div>
                    <ul>
                        {
                            categoria.length > 0 && categoria.map((item, index)=>(
                                <li><a href={`/livros/${item.categoria}/${item._id}`}><span><FaIcons.FaAngleRight/></span>{item.categoria}</a></li>
                            ))
                        }
